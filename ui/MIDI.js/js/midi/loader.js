	...
	*/
	...
	root.loadPlugin = function(opts) {
		...
		root.audioDetect(function(supports) {
			...
			/// Note: WebAudio is prioritized over WebMIDI for RedM/CEF compatibility
			/// (WebMIDI is often blocked by permissions policy in embedded browsers)
			if (supports[opts.api]) {
				api = opts.api;
			} else if (supports[hash.substr(1)]) {
				api = hash.substr(1);
			} else if (window.AudioContext) { // Chrome - prioritize WebAudio
				api = 'webaudio';
